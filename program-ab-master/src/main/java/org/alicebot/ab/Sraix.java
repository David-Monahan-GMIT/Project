...
/**
 * Class used to handle AIML \<srai\> parameters. SRAI stands for many things
 * depending on who you ask but here I am using Symbolic Reduction Artificial
 * Intelligence. These tags are used to indicate the bot should do more than
 * just return a response. This class provides utilities for Pannous and
 * PandoraBot SRAI parameters.
 * 
 * @author Dave
 *
 */
public class Sraix {
	...
	public static String sraixPannous(String input, String hint, Chat chatSession) {
		try {
			...
			// String url =
			// "https://jeannie.p.mashape.com/api?input="+input+"&locale=en_Uk&timeZone="+offset+locationString+"&login="+MagicStrings.pannous_login+"&ip="+NetworkUtils.localIPAddress()+"&botid=0&key="+MagicStrings.pannous_api_key+"&exclude=Dialogues,ChatBot&out=json";
			String url = "http://weannie.pannous.com/api?input=" + input + "&locale=en_Uk&timeZone=" + offset
					+ locationString + "&login=" + MagicStrings.pannous_login + "&ip=" + NetworkUtils.localIPAddress()
					+ "&botid=0&key=" + MagicStrings.pannous_api_key + "&exclude=Dialogues,ChatBot&out=json";

			/*
			 * From the Jeannie api on mashape HttpResponse<JsonNode> response =
			 * Unirest.get(
			 * "https://jeannie.p.mashape.com/api?input=who+created+you&locale=en&location=53.0%2C9.0&page=1&timeZone=%2B120")
			 * .header("X-Mashape-Key",
			 * "dG6NYxGf8lmshZSsiMZAPsV3VjTXp1tnTW2jsnjPnHxQ2YlThr")
			 * .header("Accept", "application/json") .asJson();
			 */

			// String url
			// ="http://weannie.pannous.com/api?input="+input+"&locale=en_Uk&login=test-user&exclude=Dialogues,ChatBot&out=json";
			// URL url = new
			// URL("https://weannie.pannous.com/api?input="+input+"&locale=en_Uk&login=test-user&exclude=Dialogues,ChatBot&out=json");
			 ...
			 * 
			 * String inputLine, page =null; while ((inputLine = in.readLine())
			 * != null) page += "\r\n" + inputLine; in.close();
			 */
			...
			if (page == null || page.length() == 0) {
				...
				if (hint.equals(MagicStrings.sraix_event_hint) && !text.startsWith("<year>"))
					...
					// log.info("Sraix: text has "+sentences.length+"
					// sentences:");
